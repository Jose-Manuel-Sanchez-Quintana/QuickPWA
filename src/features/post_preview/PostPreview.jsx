	...
	return (<>
		{
			...
			<div className="w-full">
				{
					post_data.original_post !== undefined && <p>This is a repost of {post_data.original_post.id}</p>
				}
					...
					<>
						{
							post_data.original_post === undefined ?
								<div className={post_data.media.length > 1 && 'grid grid-cols-2 gap-1'}>
									{
										post_data.media.length > 1 ?
											post_data.media.slice(0, 4).map((file) => (
												<img src={file.url} className='rounded-md bg-black h-72 object-cover object-center w-full' alt="" />
											))
											:
											post_data.media.length > 0 && <img src={post_data.media[0].url} className='rounded-md object-cover' alt="" />
									}
								</div>
								:
								<div className={post_data.original_post.media.length > 1 && 'grid grid-cols-2 gap-1'}>
									{
										post_data.original_post.media.length > 1 ?
											...
											:
											post_data.original_post.media.length > 0 && <img src={post_data.original_post.media[0].url} className='rounded-md object-cover w- max-w-xs' alt="" />
									}
								</div>
						}
					</>

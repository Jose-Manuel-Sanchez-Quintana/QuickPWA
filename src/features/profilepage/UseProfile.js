...
const UseProfile = () => {
  ...
  const user_uid =
    search_params.get("user") != null ? search_params.get("user") : user.uid;

  const getUserInfo = () => {
    // axios.get(`https://quick-api-9c95.onrender.com/user/${search_params.get("user")}`, {
    axios
      .get(
        `https://quick-api-9c95.onrender.com/user/${search_params.get("user")}`,
        {
          params: {
  ...
  const getPosts = () => {
    axios
      .get(
        `https://quick-api-9c95.onrender.com/users/${search_params.get(
          "user"
        )}/posts`,
        {
          params: {
